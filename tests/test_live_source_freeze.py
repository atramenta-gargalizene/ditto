...
class LiveSourceFreezeTest(unittest.TestCase):
    def test_frozen_regular_run_survives_live_history_changes(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertNotEqual(frozen["run_dir"], refreshed_plan["run_dir"])
            self.assertTrue(plan_path.is_file())
            self.assertTrue(segment_path.is_file())
            self.assertEqual(Path(frozen["run_dir"]), plan_path.parent)
            self.assertEqual(Path(frozen["run_dir"]) / "segments", segment_path.parent)
